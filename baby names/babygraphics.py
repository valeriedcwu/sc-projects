    ...
    for i in range(len(lookup_names)):
        for j in range(len(YEARS)):
            if str(YEARS[j]) in name_data[lookup_names[i]]:
                ...
                y_ = GRAPH_MARGIN_SIZE + CANVAS_HEIGHT / MAX_RANK * int(rank)
            else:
                rank = '*'
                y_ = CANVAS_HEIGHT - GRAPH_MARGIN_SIZE
            if j > 0:
                canvas.create_line(pre_x, pre_y, get_x_coordinate(CANVAS_WIDTH, j), y_, width=LINE_WIDTH, fill=COLORS[i % len(COLORS)])
            canvas.create_text(get_x_coordinate(CANVAS_WIDTH, j) + TEXT_DX, y_, text=lookup_names[i]+' '+rank, fill=COLORS[i % len(COLORS)],anchor=tkinter.SW)
            pre_x = get_x_coordinate(CANVAS_WIDTH, j)
            pre_y = y_
